...
export default function Gradations() {
  ...
  const handleExportExcel = () => {
    if (gradations.length === 0) {
      toast.warning("No gradations available to export.");
      return;
    }
    
    const formattedData = gradations.map(g => ({
      ID: g.id,
      "Gradation Name": g.gradation || "",
      Status: g.status === 1 ? "Active" : "Inactive"
    }));
    
    const worksheet = XLSX.utils.json_to_sheet(formattedData);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "Gradations");
    XLSX.writeFile(workbook, `gradations_export_${new Date().toISOString().split('T')[0]}.xlsx`);
    
    toast.success("Gradations exported successfully!");
  };
