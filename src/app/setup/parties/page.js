...
export default function SetupParties() {
  ...
  const fetchParties = async () => {
    setLoading(true);
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/party/read?type=${activeTab}`, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const data = await response.json();
      if (data.success) {
        toast.success(currentPartyId ? "Party updated successfully!" : "Party added successfully!");
        fetchParties();
        handleCloseModal();
      } else {
        toast.error(data.message);
  ...
  const executeDelete = async () => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/party/delete/${partyToDelete.id}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${localStorage.getItem("token")}` }
      });
      const data = await response.json();
                ...
                <tbody>
                  {parties.map((p, index) => (
                    <tr key={p.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50 transition">
                      <td className="py-3 px-4 text-center text-slate-600 font-medium whitespace-nowrap">{index + 1}</td>
                      <td className="py-3 px-4 text-slate-800 font-bold">{p.name}</td>
                      <td className="py-3 px-4 text-slate-600">{p.phone || "-"}</td>
                      <td className="py-3 px-4 text-slate-600 truncate max-w-[200px]">{p.address || "-"}</td>
                      <td className="py-3 px-4 text-center">
                        <div className="flex justify-center gap-2">
                          ...
                          <button onClick={() => confirmDelete(p)} title="Delete" className="p-2 bg-red-50 hover:bg-red-100 text-red-500 rounded-lg transition-colors">
                  ...
                  {parties.length === 0 && (
                    <tr>
                      <td colSpan="5" className="py-8 text-center text-slate-500">
                        No {activeTab === "purchase" ? "suppliers" : "customers"} found. Add one to get started.
                      </td>
                    </tr>
                  )}
                </tbody>
...
/>
                ...
                <div className="flex justify-end gap-3 mt-6 border-t border-slate-100 pt-6">
                  ...
                  >
                    ...
                    {isSubmitting ? "Saving..." : (currentPartyId ? "Update" : "Save")}
                  </button>
                </div>
              </form>
            </div>
