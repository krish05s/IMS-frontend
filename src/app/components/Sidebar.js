    ...
    <>
      ...
      <div className={`fixed top-0 left-0 h-full w-64 bg-slate-50 text-slate-700 border-r border-slate-200 p-5 z-50 shadow-sm transition-transform duration-300 md:translate-x-0 flex flex-col ${isOpen ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        <nav className="flex flex-col gap-3 flex-1 overflow-y-auto pr-2 pb-4">
          ...
          {(role === "admin" || role === "sales" || role === "purchase" || role === "user") && (
            <NavItem href="/dashboard" currentPath={pathname} onClick={() => setIsOpen(false)} icon={Icons.Dashboard} label="Dashboard" />
          )}

          {/* Gradations - Visible to admin only */}
          {role === "admin" && (
            <NavItem href="/gradations" currentPath={pathname} onClick={() => setIsOpen(false)} icon={Icons.Gradations} label="Gradations" />
          )}

          {/* Products - Visible to admin, sales, purchase */}
          {(role === "admin" || role === "sales" || role === "purchase") && (
            <NavItem href="/products" currentPath={pathname} onClick={() => setIsOpen(false)} icon={Icons.Products} label="Products" />
          )}

          {/* Purchase - Visible to admin, purchase */}
          {(role === "admin" || role === "purchase") && (
            <NavItem href="/purchase" currentPath={pathname} onClick={() => setIsOpen(false)} icon={Icons.Purchase} label="Purchase" />
          )}

          {/* Sales - Visible to admin, sales */}
          {(role === "admin" || role === "sales") && (
            <NavItem href="/sales" currentPath={pathname} onClick={() => setIsOpen(false)} icon={Icons.Sales} label="Sales" />
          )}
