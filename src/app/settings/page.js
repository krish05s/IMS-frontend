...
export default function Settings() {
  ...
  const fetchVehicles = async () => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/vehicle/read`,
        {
  ...
  const handleOpenModal = (vehicle = null) => {
    ...
    setFormData(
      vehicle
        ...
        : { vehicle_no: "", owner_name: "", status: 1 },
    );
    setIsModalOpen(true);
  };
  ...
  const handleToggle = async (vehicle) => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/vehicle/update/${encodeURIComponent(vehicle.vehicle_no)}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
              ...
              <div className="flex justify-between items-center bg-white p-6 rounded-2xl shadow-sm border border-slate-200 mb-8">
                <div>
                  ...
                  <p className="text-sm text-slate-500 mt-1">
                    Manage system configurations and data
                  </p>
                </div>
              </div>
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                  <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                    ...
                    <button
                      ...
                      className="bg-orange-500 text-white px-3 py-1.5 rounded-lg text-sm font-medium hover:bg-orange-600 transition shadow-md shadow-orange-500/20"
                    >
                      + Add Vehicle
                    </button>
                  </div>
                    ...
                    <table className="w-full text-left text-sm">
                      <thead className="bg-slate-100/50 text-slate-500 border-b border-slate-200 sticky top-0">
                        <tr>
                          ...
                          <th className="py-3 px-5 font-semibold text-center whitespace-nowrap">
                            Status
                          </th>
                          <th className="py-3 px-5 font-semibold text-right whitespace-nowrap">
                            Actions
                          </th>
                        </tr>
                      </thead>
                      <tbody>
                        {currentVehicles.map((v) => (
                          ...
                          >
                            <td className="py-3 px-5 font-medium text-slate-800 uppercase whitespace-nowrap">
                              {v.vehicle_no}
                            </td>
                            ...
                            <td className="py-3 px-5 text-center">
                              <button
                                ...
                                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${Number(v.status) === 1 ? "bg-green-500" : "bg-slate-300"}`}
                              >
                                <span
                                  className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${Number(v.status) === 1 ? "translate-x-6" : "translate-x-1"}`}
                                />
                              </button>
                            </td>
